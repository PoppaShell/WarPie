...
def save_config(
    ...
    Note: We write the POST-RENAME interface names (warpie_ap, warpie_cap0, etc.)
    not the currently detected names, because udev rules will rename them on boot.
    """
    lines = [
    ...
    for i, cfg in enumerate(configs):
        lines.extend(
            [
                f'ADAPTER_{i}_IFACE="warpie_cap{i}"',
